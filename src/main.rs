...
use buttplug::client::{device::VibrateCommand, ButtplugClient, ButtplugClientEvent};
...
async fn buttplug_task(mut switch_recv: Receiver<f64>) {
  ...
  let (client, mut event_stream) = ButtplugClient::connect_in_process("JoyconClient", 0)
    .await
    .unwrap();
  thread::spawn(|| {
    smol::run(async move {
    ...
    loop {
      ...
      match both_r.await {
        ...
        future::Either::Right((Ok(n), old_sw_r)) => {
          ...
          sw_itr_w
            .write_all(&controller_incoming[0..n])
            .await
            .unwrap();
